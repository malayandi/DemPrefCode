...
class Visualizer(object):
    def __init__(self, dt=0.5, fullscreen=False, name='unnamed', iters=1000, magnify=1.):
        ...
        self.grass = pyglet.resource.texture('carDomain/imgs/grass.png')
        ...
        def car_sprite(color, scale=0.15/600.):
            sprite = pyglet.sprite.Sprite(centered_image('carDomain/imgs/car-{}.png'.format(color)), subpixel=True)
            sprite.scale = scale
            return sprite
        def object_sprite(name, scale=0.15/600.):
            sprite = pyglet.sprite.Sprite(centered_image('carDomain/imgs/{}.png'.format(name)), subpixel=True)
            sprite.scale = scale
            return sprite
    ...
    def save_trajectory(self, name):
        if name == "":
            name = "car-%s-%d"%(self.name, int(time.time()))
        name += ".pickle"
